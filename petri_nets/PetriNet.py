...
class PetriNet:
    ...
    def add_place(self, name, description, tokens=0, ready_tokens=0, max_tokens=1, cooldown_ms=500) -> None:
    ...
    def add_transition(self, name, input_palces, output_places) -> None:
    ...
    def fire_transition(self, name) -> None:
        if name not in self.transitions:
            raise Exception(f"Transition {name} does not exist")
        self.transitions[name].fire()

    def reverse_fire_transition(self, name) -> None:
        self.transitions[name].reverse_fire()

    def set_info_terminal(self, info_terminal) -> None:
